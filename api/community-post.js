// Privileged Community post endpoint. This is the ONLY way a row can land in
// the `posts` table - the Postgres RLS policy for that table grants no
// client-reachable INSERT at all (see docs/community-schema.sql), so a
// user calling supabase.from('posts').insert(...) directly is rejected at
// the database level. This function verifies the caller's Supabase session,
// runs an AI safety check, and inserts using the service-role key.
...
// OPEN - Community posting must not hard-depend on an AI provider being
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const row = await insertRow(supabaseUrl, serviceRoleKey, "posts", {
      author_id: user.id,
