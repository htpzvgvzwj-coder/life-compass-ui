...
async function runCompassInsights({ force = false } = {}) {
  ...
  // Hermes-inspired "periodic nudge": only queue one when this is an
  // update to an existing understanding (oldItems.length, not the very
  // first-ever run - nothing to compare a first insight against, and
  // nudging before any baseline exists would feel premature) AND at least
  // one insight is genuinely new, not just a carried-over or reworded one.
  const added = oldItems.length ? rawInsights.filter((next) => !oldItems.some((item) => item.text.toLowerCase() === next.text.toLowerCase())) : [];
